...
function verifyFireblockResponse(signing_payload: string, unsigned_transaction_serialized: string): boolean {
  const actualBodyHash = getBodyHashHex(unsigned_transaction_serialized);
  console.log("Expected signing_payload:", signing_payload);
  console.log("Actual transaction body hash:", actualBodyHash);
  if (signing_payload !== actualBodyHash) {
    throw new Error("Signing payload doesn't match transaction body hash!");
  }
  return true;
}
...
async function main() {
  ...
  const isVerified = await verifyFireblockResponse(signing_payload, unsigned_transaction_serialized);
  if (!isVerified) {
    throw new Error("Signing payload doesn't match transaction body hash!");
  }
